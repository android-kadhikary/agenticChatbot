...
from langgraph_backend_agentic_tool import chatbot, retrieve_all_thread
...
#===================streamlit ui setup  with sidebar=================
...
if input:
    ...
    with st.chat_message('assistant'):
        ...
        # ***Below code is displaying every messgae, when its tool message, 
        # then it has many tokens, that are nit nesessary to print***
        # ai_message=st.write_stream(message_chunk.content for message_chunk,metadata in response)
        def ai_only_message_stream():
            for message_chunk,metadata in response:
                if isinstance(message_chunk, AIMessage):
                    yield message_chunk.content
        ai_message=st.write_stream(ai_only_message_stream())
